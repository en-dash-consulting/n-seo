...
acceptanceCriteria:
  - "`n-seo upgrade` detects how the engine was installed — git checkout, npm global (including a custom prefix), npm local — and performs the upgrade itself rather than printing advice. It prints the old and new versions, the section of the new CHANGELOG, the rollback command, and a reminder to restart the dashboard."
  - "`n-seo upgrade --check` reports what is available and changes nothing."
  - "a newer published version is shown on the Settings page and in `doctor`, both read from `data/update-check.json` rather than the network, and both compare against the engine running now — so an upgrade stops the notice immediately rather than at the next daily run."
